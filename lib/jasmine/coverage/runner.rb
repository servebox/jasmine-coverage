  ...
  module Coverage
    module Runner
      ...
      def self.run(options={})
        ...
        options[:sources_dir] ||= 'app/assets/javascripts'

        # Compile coffee script files
        puts "Compile coffee scripts from #{options[:sources_dir]}"
        Dir.glob("#{options[:sources_dir]}/**/*.coffee").each do |file|
          Jasmine::Headless::CoffeeScriptCache.new(file).handle
        end

        # Instrument the source files into the instrumented folders
        folder=File.expand_path(options[:sources_dir], "#{Jasmine::Headless::CoffeeScriptCache.cache_dir}/#{Jasmine::Headless::CoffeeScriptCache.cache_type}")
        instrumented_folder=instrumented_dir+'app'
        instrument(folder, instrumented_folder)
        # Also hoist up the eventual viewing files
        FileUtils.mv(Dir.glob(instrumented_folder+'/jscoverage*'), output_dir)

        Jasmine::Coverage::Config.resources = {folder => instrumented_folder}
        Jasmine::Coverage::Config.output_dir = output_dir
        test_rig_folder = "#{Jasmine::Coverage::Config.output_dir}/test-rig"
        ...
        options= {:reporters => [['File', "#{output_dir}/rawreport.txt"]]}.merge options
        # Run Jasmine using the original config.
        status_code = Jasmine::Headless::Runner.run options
        errStr = <<-EOS
...
**********************************************************************************************
  ...
  EOS
      ...
      def self.instrument folder, instrumented_sub_dir
        ...
        puts "jscoverage -v #{folder} #{instrumented_sub_dir}"
